...
let xibFile = CommandLine.arguments[1]
...
let parser = XibParser(filePath: xibFile, targetElement: "fontDescription") { parser, tags in
    ...
    print("start Serialize \(xibFile)")
    let output = serializer.output()

    let url = URL(fileURLWithPath: xibFile)
    do {
        try output.write(to: url, atomically: false, encoding: .utf8)
    } catch {
        print(error)
    }
    print("end Serialize \(xibFile)")
}
